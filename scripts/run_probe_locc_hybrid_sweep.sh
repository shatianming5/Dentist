...
run_one() {
  ...
  local aux="$4"
  local cons="$5"
  local exp="joint_locc_hybrid_fold${fold}_seed${seed}_${tag}"
  local out="runs/research_segcls_probe/locc_hybrid_joint/${exp}/metrics.json"
  if [[ -f "$out" ]]; then
  ...
  python3 scripts/phase3_train_raw_segcls_joint.py \
    ...
    --run-root runs/research_segcls_probe/locc_hybrid_joint \
    --exp-name "$exp" \
    --seed "$seed" \
    --device cuda \
    --epochs 80 \
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DONE locc hybrid sweep"
