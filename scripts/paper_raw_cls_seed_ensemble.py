...
def main() -> int:
    ap = argparse.ArgumentParser(description="Compute seed-ensemble metrics from raw_cls preds_test.jsonl.")
    ap.add_argument("--runs-root", type=Path, required=True, help="Root folder that contains exp/model/fold=*/seed=*/")
    ap.add_argument("--exp", type=str, required=True, help="Experiment name under runs-root.")
    ap.add_argument("--model", type=str, required=True, help="Model name under exp/ (e.g., pointnet).")
    ...
    if not fold_results:
        raise SystemExit("No valid folds found (check --runs-root/--exp/--model/--seeds/--folds).")
