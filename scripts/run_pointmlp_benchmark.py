#!/usr/bin/env python3
"""Launch PointMLP benchmark: 2 protocols × 5 seeds × 5 folds = 50 jobs on GPU 3."""
...
GPU = 3
RUN_ROOT = Path("runs/pointmlp_benchmark")
...
def main():
    ...
    for proto_name, proto_dir in PROTOCOLS:
        for seed in SEEDS:
            for fold in FOLDS:
                exp = f"pointmlp_{proto_name}_s{seed}_f{fold}"
    ...
    print(f"[LAUNCH] {len(jobs)} PointMLP jobs on GPU {GPU}")
    if not jobs:
        return 0
    ...
    for i, j in enumerate(jobs):
        ...
        if ret == 0:
            ok += 1
            # Read mIoU from results
            rfile = d / "results.json"
            if rfile.exists():
                r = json.load(open(rfile))
                miou = r.get("test_miou", "?")
                print(f"OK mIoU={miou:.4f}" if isinstance(miou, float) else f"OK")
            else:
