...
PMLP_DIM="${PMLP_DIM:-128}"
PMLP_DEPTH="${PMLP_DEPTH:-6}"
PMLP_K="${PMLP_K:-16}"
PMLP_FFN_MULT="${PMLP_FFN_MULT:-2.0}"

PRETRAIN_EXP="teeth3ds_fdi_pointmlp"
...
if [[ ! -f "${PRETRAIN_CKPT}" ]]; then
  ...
  python3 scripts/phase2_train_teeth3ds_fdi_cls.py \
    ...
    --balanced-sampler \
    --pmlp-dim "${PMLP_DIM}" \
    --pmlp-depth "${PMLP_DEPTH}" \
    --pmlp-k "${PMLP_K}" \
    --pmlp-ffn-mult "${PMLP_FFN_MULT}"
else
...
CFG="configs/raw_cls/exp/teeth3ds_fdi_pretrain_supcon_rotz_tta8_posfeat_ls0_dropout0p1_seg_all_cloudid_eq_xyzonly_pointmlp_n1024_v2.yaml"
EXP="teeth3ds_fdi_pretrain_supcon_rotz_tta8_posfeat_ls0_dropout0p1_seg_all_cloudid_eq_xyzonly_pointmlp_n1024_v2"
MODEL="pointmlp"
...
python3 scripts/aggregate_runs.py \
  ...
  --out paper_tables/raw_cls_summary_v18_main4_seg_all_cloudid_eq_rotz_tta8_pointmlp_xyzonly_n1024_v2.md \
  --seeds 1337,2020,2021 \
  --folds 0,1,2,3,4

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DONE aggregate -> paper_tables/raw_cls_summary_v18_main4_seg_all_cloudid_eq_rotz_tta8_pointmlp_xyzonly_n1024_v2.md"
