...
PRETRAIN_EPOCHS="${PRETRAIN_EPOCHS:-140}"
PRETRAIN_PATIENCE="${PRETRAIN_PATIENCE:-25}"

PT_DIM="${PT_DIM:-96}"
PT_DEPTH="${PT_DEPTH:-4}"
PT_K="${PT_K:-16}"
PT_FFN_MULT="${PT_FFN_MULT:-2.0}"

PRETRAIN_EXP="teeth3ds_fdi_pointtransformer"
...
CFG="configs/raw_cls/exp/teeth3ds_fdi_pretrain_supcon_rotz_tta8_posfeat_ls0_dropout0p1_seg_all_cloudid_eq_xyzonly_pointtransformer_n1024_v2.yaml"
EXP="teeth3ds_fdi_pretrain_supcon_rotz_tta8_posfeat_ls0_dropout0p1_seg_all_cloudid_eq_xyzonly_pointtransformer_n1024_v2"
MODEL="point_transformer"
...
python3 scripts/aggregate_runs.py \
  ...
  --out paper_tables/raw_cls_summary_v18_main4_seg_all_cloudid_eq_rotz_tta8_pointtransformer_xyzonly_n1024_v2.md \
  --seeds 1337,2020,2021 \
  --folds 0,1,2,3,4

echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DONE aggregate -> paper_tables/raw_cls_summary_v18_main4_seg_all_cloudid_eq_rotz_tta8_pointtransformer_xyzonly_n1024_v2.md"
